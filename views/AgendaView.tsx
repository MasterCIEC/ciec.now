...
const AgendaView: React.FC<AgendaViewProps> = ({ 
  ...
  const handleExportToIcs = () => {
    ...
    const itemsForMonth = allAgendaItems.filter(item => {
        const itemDate = new Date(item.date + 'T00:00:00Z');
        return itemDate.getUTCFullYear() === year && itemDate.getUTCMonth() === month;
    });

    if (itemsForMonth.length === 0) {
        alert('No hay actividades en el mes actual para exportar.');
        return;
    }
    ...
    let icsString = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//CIEC.Now//Event Calendar//EN',
    ];
    ...
    itemsForMonth.forEach(item => {
        ...
        let description = '';
        if (item.type === 'meeting') {
            description += `Categoría: ${getMeetingCategoryName(item.meetingCategoryId)}\\n\\n`;
        } else {
            description += `Organizador: ${getDisplayOrganizerNameForEvent(item as EventAgendaItem)}\\n\\n`;
        }
        if (item.description) {
            description += `Descripción: ${formatIcsText(item.description)}`;
        }

        const dtstart = formatIcsDateTime(item.date, item.startTime);
        
        let dtend = item.endTime ? formatIcsDateTime(item.date, item.endTime) : '';
        if (!dtend && item.startTime) {
            const startDate = new Date(`${item.date}T${item.startTime}`);
            const endDate = new Date(startDate.getTime() + 60 * 60 * 1000);
            dtend = `:${endDate.toISOString().replace(/-|:|\.\d+/g, '')}`;
        }
        if (!item.startTime) { // All-day event
        ...
        if (description.trim()) icsString.push(`DESCRIPTION:${description}`);
        if (item.location) icsString.push(`LOCATION:${formatIcsText(item.location)}`);
        if (dtstart) icsString.push(`DTSTART${dtstart}`);
        if (dtend) icsString.push(`DTEND${dtend}`);
    ...
    link.setAttribute("download", `CIEC_Agenda_${monthName}_${year}.ics`);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
       ...
       <div className="flex flex-col md:flex-row justify-between items-center gap-4 p-3 bg-white dark:bg-gray-800/50 rounded-lg shadow-md mb-4">
          ...
          <div className="flex items-center gap-2">
              ...
              <Button onClick={handleExportToIcs} variant="secondary" size="md" className="!p-2.5" aria-label="Exportar mes actual a archivo .ics">
